...
describe("SuperDapp::complexTest", function () {
    ...
    it("check::modifier::onlyFromSuperDapp", async () => {
        await expect(groupManager.connect(user1).createGroup(
            intToBn(1),
            1,
            1,
            ...
            86,
            40,
            { protocolFeePercent: 2, subjectFeePercent: 4 }
        )).to.be.revertedWith("Only SuperDapp can call this function");
    });
    ...
    describe("test::mainFlow", async () => {
        ...
        describe("test::createGroupWithBasicConfig", async () => {
            ...
            it("ensure announcement group has correct bonuses", async () => {
                ...
                expect(bonusesHours[0]).to.equal(BigInt(24 * 30));
                expect(bonusesHours[1]).to.equal(BigInt(24 * 180));
                expect(bonusesHours[2]).to.equal(BigInt(24 * 365));
            ...
            it("ensure social group has correct bonuses", async () => {
                ...
                expect(bonusesHours[1]).to.equal(2);
                expect(bonusesHours[2]).to.equal(4);
                    ...
                    describe("test::afterLock", async () => {
                        ...
                        describe("test::userJoinToGroupAndBalancesUpdated", async () => {
                            ...
                            it("ensure user 4 can join group and balances updated and leave after some time", async () => {
                                ...
                                expect(eth2Human(profitBefore.returnAmount)).to.equal(eth2Human(intToBn(8.084)));
                                await time.increase(time.duration.hours(1)); // add 1 hours and 10 seconds
                                ...
                                expect(eth2Human(profitAfter.returnAmount)).to.equal(eth2Human(intToBn(8.272)));

                                await time.increase(time.duration.hours(2)); // add 2 hours
                                ...
                                expect(eth2Human(profitAfter2.returnAmount)).to.equal(eth2Human(intToBn(8.554)));


                                await time.increase(time.duration.hours(4)); // add 4 hours
                                ...
                                expect(eth2Human(profitAfter3.returnAmount)).to.equal(eth2Human(intToBn(9.024)));
                                ...
                                expect(eth2Human(profitAfter4.realReturn)).to.equal(eth2Human(intToBn(9.024))); // ensure there will not be anything added


                                /** now another user will buy some shares and lets check if profit increase for user 4 */

                                const buyMoreUser6 = await bank.connect(user6).buyShares(groupId, intToBn(10));
                                //@ts-ignore
                                const buyMoreUser6Receipt = await buyMoreUser6.wait();

                                const profitBeforeUser4 = await getEstimatedSellShares(
                                    bank,
                                    user4,
                                    newTokenId,
                                );

                                expect(profitBeforeUser4.returnAmount).to.not.eq(intToBn(8.46));


                                const buyMoreUser6Again = await bank.connect(user6).buyShares(groupId, intToBn(10));
                                // @ts-ignore
                                const buyMoreUser6ReceiptAgain = await buyMoreUser6Again.wait();

                                const profitBeforeUser4Again = await getEstimatedSellShares(
                                    bank,
                                    user4,
                                    newTokenId,
                                );

                                expect(profitBeforeUser4Again.returnAmount).gt(profitBeforeUser4.returnAmount);


                                const exitTx = await bank.connect(user4).sellShares(newTokenId);

                                expect(exitTx).to.emit(bank, "SoldShares").withArgs(
                                    await user4.getAddress(),
                                    groupId,
                                    newTokenId,
                                    profitBeforeUser4Again.returnAmount,
                                );

                                expect(await groupManager.groupMembershipExists(newTokenId)).is.false;

                                expect(await bank.balanceOf(await user4.getAddress())).to.eq(
                                    intToBn(100.528)
                                );
                            });
